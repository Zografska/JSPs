...
namespace JSPs.Controllers
{
    public class BusesController : Controller
    {
        ...
        public ActionResult Route(int? id)
        {
            ...
            ViewBag.Test = db.BusStops.Where(x => x.Buses.Any(y => y.ID == id)).ToList();
        ...
        public ActionResult Route([Bind(Include = "BusId,BusStopId,BusStops,Bus")] BusBusStops bus)
        {
            if (ModelState.IsValid)
            {
                ...
                db.Buses.Find(bus.BusId).BusStops.Add(db.BusStops.Find(bus.BusStopId));
                db.SaveChanges();
                return RedirectToAction("Route", new { id = bus.BusId });
            }
            return View(bus);
        }
